...
const StandardErrors = SpecTools.standard_errors;
...
class AdvancedCCMImpl extends SimpleCCMImpl {
    ...
    createMessage( as, ctx, params ) {
        ...
        // ---
        ctx.expect_response = info.funcs[ name ].expect_result;
    ...
    onMessageResponse( as, ctx, rsp ) {
        ...
        if ( ( typeof resvars === 'string' ) || ( resvars instanceof Array ) ) {
            // New feature since FTN3 1.7
            if ( !SpecTools.checkCompiledTypeVariant( info, resvars, result ) ) {
                as.error( InternalError,
            ...
            // TODO: remove unknown for security reasons
            for ( let k in result ) {
