...
NativeIface.prototype =
{
     ...
     */
    call : function( as, name, params, upload_data, download_stream )
    {
        var ctx = {
            name : name,
            info : this._raw_info,
        ...
        as.add(
            function( as, req )
            {
                as.setTimeout( ccmimpl.options[ invoker.SimpleCCM.OPT_CALL_TIMEOUT ] );
                ...
                {
                    ccmimpl.perfomLocal( as, ctx, req );
                }
                else
                {
                    as.error( futoin_error.InvokerError, 'Unknown end-poitn schema' );
                }
