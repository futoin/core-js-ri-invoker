...
module.exports = ( spectools ) => {
    ...
    const checkType = ( info, type, val ) => {
        if ( !check_type_warned ) {
            // eslint-disable-next-line no-console
            console.warn( `SpecTools#checkType() is deprecated, use SpecTools#checkCompiledType() instead` );
            check_type_warned = true;
        }

        return _checkType( info, type, val );
    };
    let check_type_warned = false;

    const checkSubType = ( info, type, val ) => {
        if ( type instanceof Array ) {
            for ( let t of type ) {
                if ( _checkType( info, t, val ) ) {
                    return true;
                }
            }
        ...
        case 'map': {
            ...
            if ( fields ) {
                for ( let f in fields ) {
                    ...
                    if ( !checkSubType( info, field_def.type, val[ f ] ) ) {
                        ST.emit( 'error',
            ...
            if ( elemtype ) {
                for ( let ft in val ) {
                    if ( !checkSubType( info, elemtype, val[ ft ] ) ) {
                        ST.emit( 'error',
        ...
        case 'array': {
            ...
            if ( elemtype ) {
                for ( let i = 0; i < val_len; ++i ) {
                    // Note, new type stack
                    if ( !checkSubType( info, elemtype, val[ i ] ) ) {
                        ST.emit( 'error',
        ...
        case 'set': {
            ...
            for ( let ii = val.length - 1; ii >= 0; --ii ) {
                ...
                if ( ( !_checkType( info, 'string', iv ) &&
                            ...
                            !Object.prototype.hasOwnProperty.call( set_items, iv ) ) {
                    ST.emit( 'error',
