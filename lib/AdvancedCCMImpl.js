...
class AdvancedCCMImpl extends SimpleCCMImpl {
    ...
    onRegister( as, info ) {
        ...
        const iface_options = info.options;

        SpecTools.loadIface( as, info, iface_options.specDirs, this._load_cache );

        as.add( ( as ) => {
            if ( !iface_options.prodMode ) {
                SpecTools.checkConsistency( as, info );
            }

            info.coder = MessageCoder.get(
                iface_options.coder ||
                ( ( 'BinaryData' in info.constraints ) && this.options.binaryCoder ) ||
                this.options.defaultCoder
            );
        } );
    }
    ...
    createMessage( as, ctx, params ) {
        ...
        if ( info.creds !== null ) {
            if ( info.creds_master ) {
                as.error( FutoInError.InvokerError,
                    'MasterService support is not implemented' );
    ...
    onMessageResponse( as, ctx, rsp ) {
        ...
        if ( info.creds_master ) {
            ...
            try {
                ...
                as.error( FutoInError.SecurityError,
                    `Missing response HMAC: ${name}()` );
            }
