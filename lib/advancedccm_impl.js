...
var spectools =
{
    loadSpec : function( as, info, specdirs )
    {
        ...
        var m = raw_spec.inherit.match( invoker.SimpleCCM._ifacever_pattern );

        if ( m === null )
        {
            as.error( invoker.futoin_error.InvokerError, "Invalid inherit ifacever: " + raw_spec.inherit );
        }
        ...
        for ( f in sup_info.funcs )
        {
            ...
            var sup_params_keys = sup_params.keys();
            var params_keys = params.keys();
...
AdvancedCCMImpl.prototype =
{
    ...
    {
        spectools.loadSpec( as, info, this.options[ invoker.AdvancedCCM.OPT_SPEC_DIRS ] );
    },
    ...
    {
        if ( !this.options[ invoker.AdvancedCCM.OPT_PROD_MODE ] )
        {
